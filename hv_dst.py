...
if __name__ == "__main__":
    ...
    h= open("h_learned.csv", "w")
    h.write("action,hypervolume\n")
    h.write("0.0,0.0\n")
    ...
    for i in range(89):
	    ...
	    rel_path = "Run"+str(j+1)+"/learning_values-" + str((i+1)*10000) + ".txt"
            abs_file_path = os.path.join(script_dir, rel_path)
	    
	    f = open(abs_file_path)
	    
            front = [ map(float,line.split(',')) for line in f ]
            f.close()
            h_value = h_value + hv.compute(front)

        h.write(str((i+1)*10000)+ "," + str(h_value/50) + "\n")
